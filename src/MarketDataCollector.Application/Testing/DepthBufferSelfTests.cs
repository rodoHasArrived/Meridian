...
public static class DepthBufferSelfTests
{
    ...
    private static void Test_InsertUpdateDelete()
    {
        ...
        if (r1 != DepthIntegrityKind.Ok || snap1 is null) throw new Exception("Insert bid failed.");

        var r2 = buf.Apply(new MarketDepthUpdate(ts, sym, 0, DepthOperation.Insert, OrderBookSide.Ask, 101, 12), out var snap2);
        if (r2 != DepthIntegrityKind.Ok || snap2 is null) throw new Exception("Insert ask failed.");
        ...
        if (r3 != DepthIntegrityKind.Ok || snap3 is null) throw new Exception("Update bid failed.");
        ...
        if (r4 != DepthIntegrityKind.Ok || snap4 is null) throw new Exception("Delete ask failed.");
