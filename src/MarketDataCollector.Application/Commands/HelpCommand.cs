...
EXAMPLES:
    ...
    MarketDataCollector --backfill --backfill-symbols AAPL,MSFT \
        --backfill-from 2024-01-01 --backfill-to 2024-12-31

    # Use a specific provider
    MarketDataCollector --backfill --backfill-provider alpaca \
        --backfill-symbols SPY --backfill-from 2024-01-01

    # Backfill multiple symbols
    MarketDataCollector --backfill --backfill-symbols AAPL,MSFT,GOOGL,AMZN,META
...
EXAMPLES:
    ...
    MarketDataCollector --symbols-add SPY,QQQ --no-depth

    # Add with custom depth levels
    MarketDataCollector --symbols-add ES,NQ --depth-levels 20

    # Update existing symbol configuration
    MarketDataCollector --symbols-add AAPL --no-depth --update

    # Remove symbols
    MarketDataCollector --symbols-remove TSLA,NFLX
...
EXAMPLES:
    ...
    MarketDataCollector --validate-config --config ./my-config.json

    # Generate a template
    MarketDataCollector --generate-config --template alpaca
");
    }
...
EXAMPLES:
    ...
    MarketDataCollector --package --package-name q1-2024 \
    ...
    # Import a package
    MarketDataCollector --import-package ./packages/q1-2024.zip --merge
");
    }
...
EXAMPLES:
    ...
    MarketDataCollector --check-schemas --max-files 500
");
    }
...
USAGE:
    ...
    MarketDataCollector --help <topic>
