...
/// Owns all wizard state, validation, and pre-export checks so the code-behind is thin.
/// </summary>
public sealed class AnalysisExportWizardViewModel : BindableBase, IDataErrorInfo
{
    ...
    /// <summary>
    /// Pre-export validation report displayed on the review step.
    /// </summary>
    ...
    /// <summary>
    /// Estimated file size for the export.
    /// </summary>
    ...
    public void GoNext()
    {
        ...
        if (CurrentStep < 3)
        {
            ...
            // Run pre-export validation when entering the review step
            if (CurrentStep == 3)
            {
        ...
        // On the review step, only allow export if pre-export validation passed
        if (!_preExportPassed)
        {
