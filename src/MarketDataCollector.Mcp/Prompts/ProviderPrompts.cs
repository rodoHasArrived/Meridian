...
public sealed class ProviderPrompts(RepoPathService repo)
{
        ...
        [Description("Brief description of what data this provider supplies")] string description)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("Every provider in MarketDataCollector MUST comply with these ADRs:");
        sb.AppendLine();
        ...
        if (File.Exists(templatePath))
        {
            ...
            sb.AppendLine();
            sb.AppendLine($"Copy this from `src/MarketDataCollector.Infrastructure/Adapters/_Template/{templateFile}`");
        ...
        sb.AppendLine("- [ ] Test file created in `tests/MarketDataCollector.Tests/Infrastructure/Providers/`");
