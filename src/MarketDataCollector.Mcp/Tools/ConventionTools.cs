...
public sealed class ConventionTools(RepoPathService repo)
{
    ...
    [Description("Get all must-follow C# coding conventions for MarketDataCollector: async patterns, logging, DI, sealed classes, ADR attributes.")]
    public string GetCodingConventions()
    {
        var sb = new StringBuilder();
        sb.AppendLine("# MarketDataCollector C# Coding Conventions\n");
    ...
    [Description("Get the list of anti-patterns to avoid in MarketDataCollector with explanations of why each is harmful.")]
    public string GetAntiPatterns()
    {
        var sb = new StringBuilder();
        sb.AppendLine("# MarketDataCollector Anti-Patterns\n");
