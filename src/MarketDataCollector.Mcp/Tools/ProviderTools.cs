...
public sealed class ProviderTools(RepoPathService repo)
{
        ...
        [Description("Provider type: streaming | historical | symbol-search | factory | config | constants")] string type)
    {
        ...
        sb.AppendLine($"**Path in repo:** `src/MarketDataCollector.Infrastructure/Adapters/_Template/{fileName}`\n");
        sb.AppendLine("Copy this file to `src/MarketDataCollector.Infrastructure/Adapters/{YourProvider}/` and replace all `Template` references with your provider name.\n");
