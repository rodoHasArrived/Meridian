...
5. **Integrate the configuration** — follow the instructions at the top of `TemplateConfig.cs`:
   - Add the backfill config record to `BackfillConfig.cs` (in `src/MarketDataCollector.Core/Config/`).
...
7. **Write tests** — place them in `tests/MarketDataCollector.Tests/Infrastructure/Providers/`.
...
- `*Config.cs` — records live in `MarketDataCollector.Application.Config` after integration;
...
| Config / options pattern | `src/MarketDataCollector.Core/Config/BackfillConfig.cs` |
