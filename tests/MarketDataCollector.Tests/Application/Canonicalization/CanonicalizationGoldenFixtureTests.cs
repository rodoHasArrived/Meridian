...
public sealed class CanonicalizationGoldenFixtureTests
{
    ...
    static CanonicalizationGoldenFixtureTests()
    {
        ...
        while (candidate is not null && !File.Exists(Path.Combine(candidate.FullName, "MarketDataCollector.sln")))
            candidate = candidate.Parent;

        FixturesDir = Path.Combine(
            candidate?.FullName ?? assemblyDir,
            "tests", "MarketDataCollector.Tests", "Application", "Canonicalization", "Fixtures");
    }
