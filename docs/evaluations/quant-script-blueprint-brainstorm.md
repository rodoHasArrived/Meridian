...
|---|---|---|
| `Test.Run(request, strategy)` — backtest bridge | **High value** | This is MDC's differentiator vs. Jupyter — data collection + backtesting in one app. But the current interface is too verbose (see Idea #3). |
...
**Competitive signal.** QuantConnect Research, Databento's Python notebooks, and every serious quant platform use cell-based execution. Not offering it positions MDC as a toy. This is the single highest-impact design change.
...
*Generated by mdc-brainstorm. This evaluation should be read alongside `docs/plans/quant-script-environment-blueprint.md` and used to revise the blueprint before implementation begins.*
